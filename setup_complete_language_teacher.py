#!/usr/bin/env python3
"""
Language Teacher Complete Setup Script
...
class LanguageTeacherSetup:
    ...
    def check_ollama_installed(self):
        ...
        try:
            ...
            self.log("❌ Ollama command timed out", "ERROR")
            return False
    ...
    def start_ollama_service(self):
        ...
        try:
            ...
            if response.status_code == 200:
                self.log("✅ Ollama service is already running")
                return True
        except:
            pass
        ...
        try:
            ...
            self.log(f"❌ Error starting Ollama service: {e}", "ERROR")
            return False
            ...
            if response.status_code == 200:
                ...
                for model in models:
                    if self.model_name in model.get("name", ""):
                        self.log(f"✅ Model {self.model_name} is already installed")
                        return True
                
                self.log(f"❌ Model {self.model_name} is not installed")
                return False
            else:
                self.log(f"❌ Failed to check models: HTTP {response.status_code}", "ERROR")
                return False
    ...
    def download_model(self):
        ...
        try:
            ...
            if process.returncode == 0:
                self.log(f"✅ Model {self.model_name} downloaded successfully")
                return True
            else:
                self.log(f"❌ Model download failed with return code {process.returncode}", "ERROR")
                return False
    ...
    def test_model(self):
        ...
        try:
            ...
            self.log(f"❌ Model test failed: HTTP {response.status_code}", "ERROR")
            return False
            
        except Exception as e:
            self.log(f"❌ Error testing model: {e}", "ERROR")
            return False
...
"""
        ...
        try:
            ...
            self.log(f"✅ Startup script created: {script_path}")
            return True
        except Exception as e:
            self.log(f"❌ Error creating startup script: {e}", "ERROR")
            return False
    ...
    def run_setup(self):
        ...
        if not self.check_ollama_installed():
            self.log("❌ Setup failed: Ollama is not installed", "ERROR")
            self.log("Please install Ollama from: https://ollama.ai", "ERROR")
            return False
        
        # Step 2: Start Ollama service
        if not self.start_ollama_service():
            self.log("❌ Setup failed: Cannot start Ollama service", "ERROR")
            return False
        ...
        if not self.check_model_installed():
            ...
            if not self.download_model():
                self.log("❌ Setup failed: Cannot download model", "ERROR")
                return False
        
        # Step 5: Test model
        if not self.test_model():
            self.log("❌ Setup failed: Model test failed", "ERROR")
            return False
        
        # Step 6: Install Python dependencies
        if not self.install_python_dependencies():
            self.log("❌ Setup failed: Cannot install Python dependencies", "ERROR")
            return False
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("\n❌ Setup failed. Check the log above for details.")
